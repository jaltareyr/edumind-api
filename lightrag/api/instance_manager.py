...
def create_multi_workspace_app(args):
    ...
    app = FastAPI(prefix="/api", **app_kwargs)
    ...
    async def redirect_to_webui():
        """Redirect root path to /edumind_webui"""
        return RedirectResponse(url="/edumind_webui")
    ...
    class SPAStaticFiles(StaticFiles):
        async def get_response(self, path: str, scope):
            try:
                ...
                # If path isn't a real file (e.g., /edumind_webui/questions),
            ...
            # Cache rules (path is RELATIVE to the mount)
            if path.endswith(".html") or path == "" or path == "/":
                # index.html or any HTML: no cache
            ...
            # (Optional) explicit content types
            if path.endswith(".js"):
                response.headers.setdefault("Content-Type", "application/javascript")
            elif path.endswith(".css"):
                response.headers.setdefault("Content-Type", "text/css")
    ...
    class SmartStaticFiles(StaticFiles):  # Renamed from NoCacheStaticFiles
        async def get_response(self, path: str, scope):
            ...
            if response.status_code == 404 and scope["method"] == "GET":
                response = await super().get_response(self.index_file, scope)
    ...
    static_dir = Path(__file__).parent / "edumind_webui"
